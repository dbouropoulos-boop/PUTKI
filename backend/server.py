...
# ── AI-assisted streamer meta drafting (Sprint follow-up · 2026-05-19) ──

@api_router.get("/admin/streamer-meta/v2")
async def admin_list_streamer_meta_v2(_: bool = Depends(require_admin)):
    """Status-aware listing for the back-office. Returns every row with
    its derived status (NO_META | DRAFT_NEEDS_REVIEW | PUBLISHED |
    SUPPRESSED) plus the draft / published / suppression fields."""
...
@api_router.post("/admin/streamer-meta/generate-draft")
async def admin_generate_streamer_meta_draft(
    ...
    if not result.get("ok"):
        ...
        # Returning a JSON body even on error so the UI can show the
        # reason in a status pill rather than a generic toast.
...
@api_router.post("/admin/streamer-meta/publish")
async def admin_publish_streamer_meta(
    ...
    try:
        # `published_by` is the admin token (or a stable hash of it). The
        # token IS the identity at this point — we have no multi-user auth
        # for the back-office yet. We store the first 4 chars + length so
        # the audit log is useful but doesn't leak the full secret.
...
@api_router.post("/admin/streamer-meta/suppress")
...
@api_router.get("/admin/streamer-meta/history/{platform}/{user_login}")
async def admin_streamer_meta_history(
    platform: str, user_login: str, limit: int = 50,
    _: bool = Depends(require_admin),
):
