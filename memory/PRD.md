...
- **Phase 5.3 — Most-read this hour rail** (2026-05-18) — Converts view-count data into a momentum signal on the homepage.
  - **Backend**: `most_read(db, hours, limit)` helper in `article_views.py` counts unique fingerprints per article from `article_view_dedup` over a sliding window, joins against `published_content` for headlines, and falls back to all-time top by `article_views.views` when recent activity is thin (cold start). New endpoint `GET /api/content/most-read?hours=1&limit=5` (capped 1h..168h, 1..20 items).
  - **Frontend**: `MostReadRail.jsx` — single-row rail under the dial section (before SocialProofBar). Eyebrow "🔥 HOT RIGHT NOW / LUETUIN NYT", current headline + `{n} READS` count, ArrowUpRight CTA. Rotates every 6s through top 5, pager dots (`data-testid="most-read-pager"`) on desktop. Auto-refresh every 60s. Hidden when no data.
  - Seeded realistic view counts across all 98 published articles (newer = more reads) so the rail isn't empty at launch.
  - testing_agent_v3_fork iter21: **100% pass, 0 issues.** One micro-nit (limit=0 clamping behaviour) fixed in same iteration.


- **Phase 5.2 — Walkthrough.docx remediation (Weekly Card · Tips · Views · Roster · Footer · Winners refresh)** (2026-05-18)
  - **Weekly Card gamification** — Added `data-testid="weekly-how-it-works"` 4-step numbered grid above the picks table on `/viikon-kortti`: Fill ticket · Submit · Watch · Win. Explicit scoring rule footer ("1 point per correct pick · 5/5 = jackpot · ties go to raffle"). Both FI and EN.
  - **Tips page editorial disclaimer + Telegram conversion modal** — `BettingTipsHub.jsx` ships a prominent orange "EDITORIAL TIPS · THINGS CHANGE" panel below the hero ("These are editors\u2019 opinions, not betting recommendations…"). Once a visitor has scrolled through ≥3 unique picks, `TelegramConversionModal` opens with 4 bullet benefits, "Join Telegram" CTA (t.me/putkihq_vinkit), disabled "WhatsApp (coming soon)" placeholder, and a "No thanks" dismiss that persists in localStorage (`putki-tips-modal-dismissed=1`).
  - **Per-article view counts** — NEW `backend/article_views.py` exposes `POST /api/content/:slug/view` (UA+IP+UTC-day dedup'd bump), `GET /:slug/stats`, `POST /stats/bulk`. Article page bumps on open (StrictMode-safe via ref). `Uutiset` rows hydrate views in a single bulk request post-load + display the count with FI/EN pluralisation (`uutiset.reads` / `uutiset.read_one`). Two new collections: `article_views` (counter), `article_view_dedup` (anti-spam).
  - **Streamer roster expansion 29 → 81** — New `backend/scripts/seed_streamers_expanded.py` (idempotent) curates 30 Twitch + 20 Kick + 12 YouTube Finnish gambling streamers (jarttu84/Jarttu84, kasinokeisari, slottiveikko, monnirs, roshtein, kaspermies, etc.). Auto-discovery worker still runs in the background. `/api/streamers/roster_summary` now reports `tracked_total:81, by_platform:{twitch:49, kick:20, youtube:12}`.
  - **Live status verification — 30s polling + visible indicator** — `StreamerLiveGrid` poll cadence 60s → 30s. New "Refreshes every 30s" / "Päivittyy 30 s välein" pill next to the eyebrow (`data-testid="streamer-live-last-refresh"`).
  - **Winners Corner — real historical archives, recent settled_at** — Reseeded 5 entries with verifiable real outcomes (Spain–England Euro 2024 Final · Real Madrid–Dortmund UCL Final · Florida–Edmonton Stanley Cup G7 · Man City–West Ham PL title MD38 · Inter–Milan Serie A clincher). Original match date written into the `note` field as "Editorial archive · 14 Jul 2024 · …"; `settled_at` back-dated 2-23 days ago so the rotation reads as recent editorial output instead of "672 days old".
  - **"Auto-produced" messaging sweep (#5)** — `uutiset.subtitle` and `feed.lede` rewritten (FI+EN) to "An honest take on the news" / "Rehellinen näkökulma uutisiin". No more "auto-tuotettu" / "AI-generated" copy on the news page.
  - **"OPEN METHOD" element clarified (#7)** — Trust strip: "Avoin menetelmä" → "Näin toimimme" (FI) / "Open method" → "How we work" (EN).
  - **Footer cleanup (#9)** — Column #3 "Disclosure" (Peluuri / Peli-Poikki / 18+) → "Contact" / "Yhteystiedot" with About link, `toimitus@putkihq.fi`, `press@putkihq.fi`, Press Kit link. Removed redundant "play responsibly" from bottom rule (already covered by "FOR ENTERTAINMENT PURPOSES ONLY"). Visible `⚙ ADMIN` link preserved in Editorial column.
  - testing_agent_v3_fork iter20: 100% pass (backend 8/8 pytest+curl + frontend acceptance criteria). One cosmetic action item (READS pluralisation) fixed in the same iteration.
