...
- **Phase 4 Pre-Launch — Bilingual completion + UX conversion pass** (2026-05-18) — (1) Wired `t()` through every previously-hardcoded Finnish page: `/peli`, `/tietoa-meista`, `/menetelma`, `/viikon-kortti`. Wired `LiveActivityFeed`, `PersistentCapture`, `Footer` page links, `DialCockpit` last-update ticker, `Layer2StatusPanel` admin times. Added centralized `formatTimeAgo()` / `formatLiveDuration()` / `formatKickoff()` helpers in `/app/frontend/src/utils/formatTime.js`. (2) UX conversion pass — Batch A: hero value-prop + primary `SET ALERT` CTA + `SocialProofBar` (4 honest counters: subscribers, articles today, streamers tracked, picks live). Batch B: bulk wildcard "Alert me on any Finnish streamer going live" card at end of streamer grid + `PaivaVitoset` track-record strip (honest "Results begin this week" empty state). Batch C: `StickyTelegramCTA` (fade-in after 600px scroll, 7-day dismiss) + friendlier empty states on Vitoset / streamer grid / activity feed. Batch D: `DialHelp` `?` tooltip next to PUTKI HQ MITTARI brand label + `putki-feed-pulse` 1.6s amber-flash animation on freshly-arrived feed items. testing_agent_v3_fork iter17: 100 % pass on all 13 acceptance criteria, 0 console errors, FI/EN flip clean across desktop + mobile.
