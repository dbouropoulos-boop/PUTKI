...
- **Phase 4 Week 1 — Layer 2 Backend + SSE Dial** (2026-05-17) — Event-responsive engine landed. Four async pollers in `backend/layer2_workers.py` (Twitch 60s, Reddit 1h, NHL 5m, RSS 15m) writing summary docs to four new MongoDB collections (`stream_signals`, `social_signals`, `sports_signals`, `news_signals`) with 14-day TTL indices on `expires_at`. `dial_engine.py` fully rewritten — old 6-signal formula gone, replaced with the locked 4-signal weighted composite (40% stream / 30% social / 20% sports / 10% news). Intensities are log-scaled for Twitch viewers, linear-scaled for Reddit (saturate at 20 mentions), binary for NHL games (Phase 1 = all NHL games per user revision), linear for RSS hits (saturate at 10 matches). New `backend/dial_sse.py` in-process pub-sub broadcaster; `GET /api/dial/stream` Server-Sent-Events endpoint sends bootstrap snapshot + per-tick updates + 15s heartbeats. New admin endpoints: `GET /api/admin/layer2/status` (per-collection doc counts + latest summary + SSE subscriber count) and `POST /api/admin/layer2/tick?worker=` (manual force-tick + dial recompute + SSE publish). Layer 2 workers wired into startup with `PUTKI_HQ_DISABLE_LAYER2=1` master kill switch + per-worker disables (`PUTKI_HQ_DISABLE_TWITCH_POLLER`, ..._REDDIT_POLLER, ..._NHL_POLLER, ..._RSS_POLLER). Legacy `_signal_dial_worker` kept for back-compat with the `signals` collection but no longer drives the dial. 15 new pytest cases in `tests/test_phase4_w1_layer2.py` covering formula edges, mocked tick functions, SSE chunk format, admin endpoints. **139/139 backend regression green.** Reddit returns 403 from datacenter IPs (known limitation, OAuth fallback documented but not blocking).
